<template>
  <gov-table>
    ...
    <template slot="body">
      <gov-table-row v-for="serviceLocation in serviceLocations" :key="serviceLocation.id">
        ...
        <gov-table-cell right>
          <gov-link :to="{ name: 'service-locations-show', params: { serviceLocation: serviceLocation.id } }">View</gov-link>
        </gov-table-cell>
      </gov-table-row>
